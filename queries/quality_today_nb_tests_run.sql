/*
This query returns nb tests ran today
...
*/
select
    sum(case when te.run_started_at::date = sysdate()::date then 1 else 0 end) as today
    , sum(case when te.run_started_at::date = dateadd('day', -1, sysdate())::date
        ...
        and hour(te.run_started_at) = hour(sysdate()) and minute(te.run_started_at) >= minute(sysdate()) ) then 1 else 0 end)::number as yesterday 
from
