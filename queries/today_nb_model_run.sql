/*
This query returns nb model ran today
...
*/
select
	sum(case when me.run_started_at::date = sysdate()::date then 1 else 0 end) as today
    , sum(case when me.run_started_at::date = dateadd('day', -1, sysdate())::date
        ...
        and hour(me.run_started_at) = hour(sysdate()) and minute(me.run_started_at) >= minute(sysdate()) ) 
        then 1 else 0 end)::number as yesterday 
from
