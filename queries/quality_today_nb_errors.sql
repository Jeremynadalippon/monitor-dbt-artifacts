/*
This query returns nb tests in errors today
...
*/
select
	coalesce(sum(case when te.run_started_at::date = sysdate()::date then 1 else 0 end)::number, 0) as today
    , coalesce(sum(case when te.run_started_at::date = dateadd('day', -1, sysdate())::date
        and hour(te.run_started_at) <= hour(sysdate())and not ( te.run_started_at::date = dateadd('day', -1, sysdate())::date
        and hour(te.run_started_at) = hour(sysdate()) and minute(te.run_started_at) >= minute(sysdate()) ) then 1 else 0 end)::number, 0) as yesterday
from
